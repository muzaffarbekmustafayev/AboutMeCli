...
const PortfolioProjectCard = ({ project, onClick, t }) => {
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-[2rem] border border-slate-100 bg-white transition-all hover:border-blue-200 hover:shadow-2xl dark:border-slate-800 dark:bg-slate-900/40">
      <ProjectImage
      ...
      <div className="flex flex-1 flex-col p-6 sm:p-8">
        ...
        <div className="mb-6 flex flex-wrap gap-2 sm:mb-8">
          {project.technologies?.slice(0, 3).map((tech, idx) => (
            <span
              key={idx}
              className="rounded-xl bg-slate-50 px-3 py-1 text-[11px] font-bold text-slate-500 dark:bg-slate-800/50 dark:text-slate-400 sm:px-4 sm:py-1.5 sm:text-xs"
            >
              {tech}
            </span>
          ))}
        ...
        <div className="mt-auto flex items-center justify-between border-t border-slate-50 pt-5 dark:border-slate-800/50 sm:pt-6">
          <button
            ...
            className="text-xs font-bold uppercase tracking-wider text-blue-600 hover:text-blue-700 dark:text-blue-400 sm:text-sm"
          >
            {t("portfolio.details")}
          </button>
          ...
          <div className="flex gap-4 text-slate-400 sm:gap-5">
            {project.links?.github && (
              <a href={project.links.github} target="_blank" rel="noopener noreferrer" className="transition-colors hover:text-slate-900 dark:hover:text-white">
                <Github size={20} className="sm:size-[22px]" />
              </a>
            )}
            {project.links?.live && (
              <a href={project.links.live} target="_blank" rel="noopener noreferrer" className="transition-colors hover:text-blue-600">
